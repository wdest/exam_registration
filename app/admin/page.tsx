...
export default function LandingPage() {
  ...
  const [siteInfo, setSiteInfo] = useState({
    ...
    email: "info@moc.az",
    map_url: "https://www.google.com/maps/embed?pb=..." // Default boş olmasın deyə
  });
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // 2. Settings (DÜZƏLİŞ: 'settings' cədvəlindən oxuyuruq)
        const { data: settingsData } = await supabase.from("settings").select("*");
        
        if (settingsData) {
          const newInfo: any = { ...siteInfo };
          settingsData.forEach((item) => {
            // Sadəcə bizə lazım olan açarları götürürük
            if (["phone", "address", "email", "map_url"].includes(item.key)) {
