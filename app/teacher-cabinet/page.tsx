...
export default function TeacherCabinet() {
  ...
  const handleCreateGroup = async (e: React.FormEvent) => {
    e.preventDefault(); if (scheduleSlots.length === 0) return;
             ...
             <div className="animate-in fade-in">
                ...
                {!analyticsGroupId && (
                    <div className="h-64 flex flex-col items-center justify-center text-gray-400 border-2 border-dashed rounded-2xl">
                        ...
                        <p>Analiz etmək üçün yuxarıdan qrup seçin.</p>
                    </div>
                )}
             </div>
        )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in">
                <div className="lg:col-span-1 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border dark:border-gray-700 h-fit sticky top-24">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        {editingId && (
                            <button onClick={resetForm} className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded hover:bg-gray-200 flex items-center gap-1"><RefreshCcw size={12}/> Ləğv et</button>
                        )}
                    </div>
