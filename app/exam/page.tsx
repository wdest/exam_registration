...
export default function ExamRegister() {
  ...
  // --- YENİ HİSSƏ: İmtahanları saxlamaq üçün State ---
  const [exams, setExams] = useState<any[]>([]);
  const [selectedExam, setSelectedExam] = useState("");

  // --- YENİ HİSSƏ: Bazadan İmtahanları Çəkmək ---
  useEffect(() => {
    async function fetchExams() {
      const { data } = await supabase
        .from("exams") // 'exams' cədvəlinə baxırıq
        .select("*")
        .eq("is_active", true) // Yalnız AKTİV olanları gətiririk
  ...
  async function submitForm(e: any) {
    ...
    const formData = {
      ...
      examName: selectedExam // <-- Seçilən imtahan adını göndəririk
    };
      ...
      >
        ...
        {result ? (
          ...
          <form onSubmit={submitForm} className="space-y-4">
            ...
            {/* --- YENİ: DİNAMİK İMTAHAN SEÇİMİ --- */}
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1">Hansı imtahana yazılırsınız?</label>
              <select 
                value={selectedExam}
                onChange={(e) => setSelectedExam(e.target.value)}
                className="w-full p-3 bg-amber-50 border border-amber-200 rounded-xl text-gray-900 font-bold focus:ring-2 focus:ring-amber-500 outline-none"
                required
              >
                <option value="">Bir imtahan seçin...</option>
                {/* Əgər imtahan yoxdursa */}
                {exams.length === 0 && <option disabled>Hal-hazırda aktiv imtahan yoxdur</option>}
                
                {/* Bazadan gələn imtahanlar */}
                {exams.map(ex => (
            ...
            <select name="className" required className="w-full p-3 border rounded-xl outline-none focus:border-amber-500 bg-white">
              <option value="">Sinif seçin</option>
              {[...Array(11)].map((_, i) => (
                <option key={i} value={i + 1}>{i + 1}-ci sinif</option>
              ))}
            </select>
